jQuery ->
  ...
  $('#closeImportContact').click ->
    chng = $('#entry').val()
    dne = $('#saved').val()
    if dne == "true"
      $('#' + chng).closest('tr').fadeOut(600)
      ...
      jQuery.ajax
        ...
        success:
          (data) ->
            ...
            rptTable += "<table class='table table-bordered'><thead><tr><th>Cell Number</th><th>Keyword</th><th>Option</th><th>Short Code</th><th>ExtraText</th><th>Send Date & Time</th></tr></thead><tbody>"
            while i < data.results.length
              rptTable += "<tr>"
              rptTable += "<td>" + data.results[i].cell_number + "</td>"
              rptTable += "<td>" + data.results[i].keyword + "</td>"
              rptTable += "<td>" + data.results[i].option + "</td>"
              rptTable += "<td>" + data.results[i].short_code + "</td>"
